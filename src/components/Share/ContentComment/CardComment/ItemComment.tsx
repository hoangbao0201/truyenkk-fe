...
const ItemComment = ({
    ...
    return (
        <div className="relative">
            <div
                className={`flex item-comment relative ${isReply && "pl-12"}`}
            >
                {
                    isReply ? (
                        <>
                    ...
                    <div className="flex items-center w-full">
                        ...
                        <div className="border rounded-[20px] py-[8px] px-[12px] mb-1 bg-gray-100 dark:bg-slate-700 min-h-[50px]">
                            <div className="flex items-center">
                                <TextLevel role={comment?.sender.role.roleName} rank={comment?.sender.rank}>
                                    {comment?.sender.name}
                                </TextLevel>
                                {
                                    comment?.chapterNumber && <span className="text-blue-500 text-sm ml-2 italic">Chapter {comment?.chapterNumber}</span>
                                }
                            </div>
